...
for i = 1:length(w_vector)
    ...
    while (i > 2) && ...
            ((abs(2*solutionVector(i - 1) - solutionVector(i - 2) - solutionVector(i)) > stepSize) ...
    ...
    while (i > 2) && ...
            ((abs(2*doubleRootVector(i - 1) - doubleRootVector(i - 2) - doubleRootVector(i)) > stepSize) ...
...
fprintf('minimum load : %f \nAt frequency : %f \n', min(solutionVector), ...
    w_vector(find(min(solutionVector) == solutionVector)))
