function [detVal] = evaluate_determinant_M(w_eval, kappa_eval, lambda_eval)

if abs(lambda_eval - 1) < 5e-3
    detVal = 1e9;
    return;
end
...
% eigVals_abs = sort(abs(eigVals));
 detVal = real(det(systemMat));
% detVal = eigVals;
%detVal =  eigVals(min(abs(eigVals)) == abs(eigVals))/(prod(eigVals_abs(2:end))^2); 
end